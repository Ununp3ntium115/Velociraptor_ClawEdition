...
# Run from repo root (module in lib/ - see docs/WORKSPACE_PATH_INDEX.md)
...
try {
    ...
    Import-Module .\lib\modules\VelociraptorDeployment\VelociraptorDeployment.psd1 -Force
    ...
    try {
        ...
        $standaloneBtn.Add_Click({
            try {
                ...
                if ($choice -eq "Yes") {
                    Start-Process "powershell.exe" -ArgumentList "-ExecutionPolicy Bypass -Command `"Set-Location '$repoRoot'; Import-Module .\lib\VelociraptorSetupScripts.psd1; Write-Host 'Testing standalone deployment...'; Deploy-VelociraptorStandalone -WhatIf`""
                }
            }
            catch {
