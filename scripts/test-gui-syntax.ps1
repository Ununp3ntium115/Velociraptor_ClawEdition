...
try {
    ...
    # Test 1: Basic syntax check (canonical GUI in apps/gui after reorganization)
    $guiPath = Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'apps\gui\VelociraptorGUI.ps1'
    if (-not (Test-Path $guiPath)) { $guiPath = Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'scripts\VelociraptorGUI-InstallClean.ps1' }
    $null = Get-Content $guiPath -ErrorAction Stop
