...
if (-not (Test-Path "lib\VelociraptorSetupScripts.psd1")) {
    Write-Host "❌ Run this script from the repository root (or scripts/) - lib\VelociraptorSetupScripts.psd1 not found" -ForegroundColor Red
    exit 1
}
...
try {
    # Update module manifest (lib/ after reorganization)
    $manifestPath = "lib\VelociraptorSetupScripts.psd1"
