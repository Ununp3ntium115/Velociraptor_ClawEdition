...
final class IncidentResponseUITests: XCTestCase {
    ...
    // Note: These tests would require opening the Incident Response window
    // which may need specific menu item access or keyboard shortcuts
    
    func testIncidentResponseWindowOpens() throws {
        // Use menu to open Incident Response
        let menuBar = app.menuBars.firstMatch
        XCTAssertTrue(menuBar.exists)
        
        // This would depend on your actual menu structure
        // menuBar.menuBarItems["Window"].click()
        // app.menuItems["Incident Response Collector"].click()
    }
}
...
final class SettingsUITests: XCTestCase {
    ...
    func testSettingsHasTabs() throws {
        app.typeKey(",", modifierFlags: .command)
        
        // Verify tabs exist
        XCTAssertTrue(app.tabs["General"].waitForExistence(timeout: 5))
        XCTAssertTrue(app.tabs["Security"].exists)
        XCTAssertTrue(app.tabs["Advanced"].exists)
    }
}
